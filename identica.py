"""
Loads notices from identi.ca via the API, saves the dates of the notices and
plots them.

Now that identi.ca is switched to the pump.io engine this script is most
probably obsolete (or should be overhauled).
"""
...
def NoticeGetter(Notice):
    ...
    URL = 'http://identi.ca/api/statuses/show/' + str(Notice) + '.xml'
    ...
    # strip the UTC shift, since I just couldn't get the %z parameter to work.
    Date = str(Date[:-11])+str(Date[-5:])
    Date = datetime.datetime.strptime(Date, '%a %b %d %H:%M:%S %Y')
    print '[' + str(counter) + '/' + str(howmany) + '] ID', Notice, \
        'was posted on', Date
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print '[' + str(counter) + '/' + str(howmany) + '] ID', ID, \
