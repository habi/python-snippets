...
for Image in range(1,13):
	...
	for blendinglevel in range(1,101,10):
		# Images go from 1 to 12 for each month. We use 'Image' and 'Image + 1'
		# in each loop to construct the call for ImageMagick. Since we want to
		# merge December with January, we can simply use the Image-number 
		# modulo 12 (Image % 12), which gives 1 for 13.
		...
		# since the rotation leaves a background, we just shave 1000 px off and
		# get rid of that background. There probably is an option to 'rotate',
		# but this is the KISS version.
		blendcommand += '-shave 1000x1000'
